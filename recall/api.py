...
mcp = FastMCP(
    name="recall",
    instructions=(
        "Recall（拾忆）个人知识库。**默认先查**：凡涉及技术概念、术语、方法论的问题，"
        "先调 kb_search 取回带出处的证据再决定怎么答 —— 不必等用户说出「我的笔记」"
        "（他的知识库就是他的学习记录）。检索到的内容按 recall-assembly 规范用 [n] 标注"
        "引用作答，不要凭记忆回答笔记里的内容。"
        "⚠️ 若这一次**没有**查知识库就作答，必须在回答里**明确披露**「这条没有查你的知识库，"
        "以下是通用/联网答案」，绝不能让通用或联网内容看起来像是来自笔记。"
    ),
)
...
async def kb_search(query: str, top_k: int = 20, max_tokens: int = 3000) -> SearchResult:
    ...
    何时调用：**默认动作** —— 凡涉及技术概念、术语、方法论的问题，**先调本工具一次**
    再决定怎么答（不必等用户说出「我的笔记」）。只有问题与知识库领域**完全无关**
    （闲聊、与本机笔记主题无关的领域）时才跳过；**跳过并直接作答时，必须在回答里披露**
    「这条没有查你的知识库」，不要让通用/联网答案看起来像来自笔记（roadmap R-48）。
    调用方须按返回的 references 用 [n] 标注引用，且只依据证据回答；
    证据只「提到」某术语却没解释时的处置见 recall-assembly 规范「情形 B」。
...
async def kb_answer(query: str, max_tokens: int = 3000) -> AnswerResult:
    ...
    本工具**总是自己检索**（不会用模型记忆作答），故无需调用方额外披露；
    证据只「提到」某术语却没解释时，它会按「笔记只提及、未解释」作答（roadmap R-47）。
