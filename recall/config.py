...
实测建议值：**0.60**（笔记内 top1 最低 0.799；门槛负责的"远域 + 完全没提"最高 0.392 ⇒
空档 `[0.392, 0.799]` 的中点；2026-09-26 扩题复测）。

⚠️ 它**只**管"远域"与"完全没提"两类。第三类「提了名没解释」（术语在笔记里出现过、
但笔记没讲它是什么）的分数与笔记内题**深度重叠**（中位 0.522、最高 YaRN 0.902 ⇒ 比
30 道笔记内题里的 28 道都高），**门槛在原理上拒不掉它** —— 那类问题必须由**答案模板**
声明"笔记只提及、未解释"并转联网搜索（roadmap R-47）。详见 `eval/BASELINE.md` §7.6
与 `tech.md` §4.1、§17 决策记录 17。
"""
