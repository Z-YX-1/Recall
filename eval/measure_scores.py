...
| `adjacent` | 同上 `group=adjacent` | **邻近**：LLM 相关但笔记没写（最难） |

判读重点在 `adjacent`：远域题分数天然很低，挡不挡都不难；邻近题分数若与笔记内题
重叠，阈值方案即告失败。**邻近组才是 R-32d「硬答」问题的真身**。

## 判据

阈值 ``t`` 下：``保留率`` = 笔记内题 top1 ≥ t 的比例（越高越好，掉了就是误伤），
``拒绝率`` = 笔记外题 top1 < t 的比例（越高越好）。**可分**的判定取**最紧的一组**
（`adjacent`）：存在 ``t`` 使 ``保留率 ≥ 0.9`` 且 ``adjacent 拒绝率 ≥ 0.9``。
...
def load_out_of_vault(path: Path) -> list[dict[str, str]]:
    ...
    Raises:
        ValueError: 出现既非 ``far`` 也非 ``adjacent`` 的 group。
    """
    ...
    for number, line in enumerate(path.read_text(encoding="utf-8").splitlines(), start=1):
        ...
        if group not in {GROUP_FAR, GROUP_ADJACENT}:
            raise ValueError(
                f"{path.name}:{number} 的 group 必须是 far 或 adjacent，实得 {group!r}"
            )
...
def summarize(scores: list[QuestionScore]) -> list[GroupStats]:
    ...
    for group in (GROUP_GOLDEN, GROUP_FAR, GROUP_ADJACENT):
...
def judge(sweep: list[SweepRow]) -> tuple[str, float | None]:
    ...
    判定取**最紧的一组**（``adjacent``）：存在阈值使保留率 ≥ 0.9 且邻近拒绝率 ≥ 0.9。
    ...
    if candidates:
        ...
        return (
            ...
            f"邻近拒绝 {best.adjacent_rejected:.2%}、远域拒绝 {best.far_rejected:.2%}",
            best.threshold,
        )
    # 不可分时给出"最接近"的那一行，便于判断差多远
    closest = max(sweep, key=lambda row: min(row.golden_kept, row.adjacent_rejected))
    return (
        "不可分：找不到同时满足『笔记内保留 ≥ 90%』与『邻近拒绝 ≥ 90%』的阈值；"
        f"最接近的一行是 t={closest.threshold:.2f}"
        f"（保留 {closest.golden_kept:.2%} / 邻近拒绝 {closest.adjacent_rejected:.2%}）"
        " ⇒ 建议否决「纯阈值」路线，转其它候选",
        None,
    )
...
def recommend(
    ...
    **不动证据带** ⇒ Recall@K 与基线逐位一致，"答不答"与"引哪些"被解耦。
    ...
    Returns:
        ``(建议阈值, 说明文本)``；若两组完全重叠则为 ``(None, 说明)``。
    """
    in_vault = [item.top1 for item in scores if item.group == GROUP_GOLDEN]
    outside = [item.top1 for item in scores if item.group != GROUP_GOLDEN]
    if not in_vault or not outside:
        return None, "样本不足：需要同时有笔记内与笔记外两组问题。"
    ...
    detail = (
        ...
        f"远域拒绝 {gate.far_rejected:.2%}、邻近拒绝 {gate.adjacent_rejected:.2%}；"
...
def render(report: ScoreReport) -> str:
    ...
    labels = {GROUP_GOLDEN: "笔记内", GROUP_FAR: "远域", GROUP_ADJACENT: "邻近"}
    ...
    lines += [
        ...
        "",
        f"{'t':>5}{'笔记内保留':>12}{'远域拒绝':>10}{'邻近拒绝':>10}",
        "-" * 40,
    ]
    ...
    lines += [
        ...
        "",
        f"{'t':>5}{'R@1':>8}{'R@3':>8}{'R@5':>8}{'R@10':>8}{'远域拒':>9}{'邻近拒':>9}{'留证据':>8}",
        "-" * 66,
    ]
    for trow in report.truncation:
        if round(trow.threshold * 100) % 5 == 0:  # 每 0.05 打一行
            lines.append(
                ...
                f"{trow.adjacent_rejected:>9.2%}{trow.kept_evidence_ratio:>8.1%}"
            )
    lines += [
        "",
        "六、建议",
        ...
        "",
        "  ⚠️ 样本量说明：笔记内 30 题、笔记外 21 题（其中邻近仅 6 题）。空档宽度可信，",
