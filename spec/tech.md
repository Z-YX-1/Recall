...
| **B. 提了名没解释** | 证据里**出现了**问的那个术语，但**没有解释它**（只出现在一句话 / 一个列表 / 一张对照表 / 一道选择题的选项里） | **生成侧**：回答模板（**R-47**） | `recall/llm.py::SYSTEM_PROMPT` 第 3 条＋`recall/assemble.py::FIDELITY_RULES` 第 3 条＋`skill/recall-assembly.md`「情形 B」＋`ASSEMBLY.md`（后两者同一份，须同步到 `$DSH_HOME/skills/`） |
...
- **候选缓解（未实施，需项目工程师批准）**：把"建 payload 索引失败"从**致命**降为
  **告警并继续** —— 索引是**查询性能**优化，不是检索正确性的前提；但会让过滤检索变慢，
  属行为变更，故不擅自改。
...
（**默认只列出**，确认后 `--yes`）；④ 保持 D 盘余量充裕；⑤（**需批准**）把"建 payload 索引失败"
从致命降为告警并继续 —— 索引是查询性能优化、不是检索正确性前提。清理时**尽量在 Qdrant 停止后**
进行：运行时删目录是否留下失效句柄**尚未证实**（`os error 5 = 拒绝访问`与"目录不存在"的
