...
- **当前步骤**：**R-47 实施完成（2026-09-26）** —— 「提了名没解释」规则已同步进
  **四处模板**（`llm.py::SYSTEM_PROMPT` 第 3 条、`assemble.py::FIDELITY_RULES` 第 3 条、
  `skill/recall-assembly.md` + `ASSEMBLY.md` 的「情形 B」，并已同步到 `$DSH_HOME/skills/`），
  新增 `tests/test_answer_template.py`（5 项）与 `tools/verify_r47.py`；
  **R-42 取值定为 0.60 并已写入 `.env`**（重启 API 生效；进程内核验 `= 0.6`）。
  ⇒ **待你：重启 `recall.api` 后跑 `tools\verify_r47.py` 与 `tools\verify_phase6.py --probe-vault`，
  并在 DSH 里问那 10 道 `mentioned` 题验「联网补充」那半**
...
- **待请示事项**（以下为**非阻塞**的后续选择）：
  - **需你拍板（2026-09-26 收敛后只剩 4 项）**：
    1. **重启 API 一次**（一步生效两件事）：`python -m recall.api` ⇒ ① `RECALL_EVIDENCE_MIN_SCORE=0.60`
       生效；② R-47 新提示词生效。随后跑 `python tools\verify_r47.py` 与
       `python tools\verify_phase6.py --probe-vault`（期望 **16/16**，且 R-42 一节显示"门槛 0.60 + 空证据"）。
    2. **R-47 的后半段只能在 DSH 里验**（人工、不可替代）：把 `tools\verify_r47.py` 打印的 10 道
       `mentioned` 题各问一遍，答案须**同时**出现"笔记只提及 / 未解释"与**联网补充**内容；
       顺带完成那条老判据——问一句笔记里的内容确认能命中。
    3. **R-39 公网接入**：选路线（**A. Cloudflare Tunnel（推荐）** / B1. 云服务器反代 / B2. 整体上 GPU 云）
       ＋ 域名与运营主体谁出。手册 `docs/R-39-public-access.md` **已回写为最新**（§8 三项待办均已实现
       ⇒ **无需立项**，只剩选路线 + 开隧道 + 配 key 表 + 验收）。
       硬约束：给 Coze 的 token 要么映射到 `me`、要么先给笔记写 `visibility: public`；
       **不能为公网另起实例**（单份模型 ≈4.5GB 显存）。
...
- **本文件版本**：v0.27.0（2026-09-26 **项目工程师三项决策全部落地**：① **R-42 取 `0.60`**
  —— 已写入 `.env`，重启即生效；② **R-47 批准并实施** —— 「提了名没解释」规则同步进
  `llm.py::SYSTEM_PROMPT`、`assemble.py::FIDELITY_RULES`、`skill/recall-assembly.md`（已同步到
  `$DSH_HOME/skills/`）与 `ASSEMBLY.md`，新增 `tests/test_answer_template.py` + `tools/verify_r47.py`；
  ③ **R-40 真实验收「下次再说」**。另：**根治了 `.env` 泄漏进用例**的第三次假红
  （新增 `RECALL_SKIP_DOTENV`）；**更正**了"Qdrant 1.19.1 已彻底解决 IO 故障"的过满结论。
  **当前待项目工程师：① 重启 `recall.api`（0.60 + 新提示词一起生效）→ 跑 `tools\verify_r47.py`
  与 `tools\verify_phase6.py --probe-vault`；② 在 DSH 里问 10 道 `mentioned` 题验联网那半；
