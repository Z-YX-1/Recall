...
- **当前阶段**：✅ **Phase 0~5 全部完成（首版交付验收达成）+ R-43c / R-45 已闭环**；
  **Phase 6 进行中：R-38 已真实验收通过（16/16）、R-42 已实现+测量完成待拍板阈值、
  R-40 待验收（未配 key 表时等于只验了 S1）；R-39 / R-41 待定方向**
- **当前步骤**：**R-42 阶段 2 完成（2026-09-26）** —— 把「笔记外」拆成 **完全没提**(10 题) 与
  **提了名没解释**(10 题) 复测：门槛负责的两类与笔记内空档 `[0.392, 0.799]` ⇒ 建议值
  **0.58 → 0.60**；而"提了名没解释"与笔记内分数带**深度重叠**（最高 YaRN 0.902），
  **门槛拒不掉 ⇒ 已登记 R-47 交答案模板**（§五）；**待你决定是否把 0.60 写进 `.env`**
...
- **待验收项**：**R-40**（权限 S2）—— 实现与 gates 均已完成；⚠️ **未配 `RECALL_API_KEYS` 时
  强制路径根本没被触发**（脚本走的是"未配置⇒不鉴权"的 S1 分支）⇒ 要算真验收，得配一份 key 表
  再跑 `tools\verify_phase6.py --api-key <token>`，那时 401/200 与"配置-运行态一致性"检查才会真跑
- **待拍板项**：**R-42 收尾**（是否把 `RECALL_EVIDENCE_MIN_SCORE=0.60` 写进 `.env`；
  **不写就完全维持现状**，代码无需回滚）；**R-47**（答案模板的"提及未解释"规则，§五）
...
- **待请示事项**（以下为**非阻塞**的后续选择）：
  - **需你拍板**：
    ...
    5. **R-42 收尾（只剩一个动作）**：证据门槛已实现、已 A/B 验证，**默认关闭**。
       请定：**是否把 `RECALL_EVIDENCE_MIN_SCORE=0.60` 写进 `.env`**（扩题后中点由 0.58 上移；
       0.58 在新题集上**仍安全**，只是余量变小）。
       效果：**远域 + 完全没提**从"回一堆低相关片段"变成**空证据**（`kb_search` 空包、
       `kb_answer` 直接答"笔记里没有"且**不调 LLM**）—— 这两类 **25/25 全拒、
       笔记内 0/30 误拦、Recall@K 逐位不变**。**不写就完全维持现状**，代码无需回滚。
       ⚠️ 但门槛**拒不掉"提了名没解释"**（10 题只拒掉 5 题，YaRN 高达 0.902）⇒ 这类问题的
       "不要硬答"**必须**靠答案模板，已登记为 **R-47 待你批准**。
       详见 `eval/BASELINE.md` §7.6 与 tech.md §4.1。
...
- **最近一次测试结果**（2026-09-26）：✅ **`pytest` 全绿 —— 293 passed**（新增
  `tests/test_eval_scores.py` **6 项**；本轮**第一次**全量跑出现 1 项偶发失败，单跑即过、
  复跑全绿，详见本文件 §七 末条）；`ruff` 零告警；`mypy` strict **54 文件**零错误。
...
- ⚠️ **已定位但未解决（2026-09-25）**：**每次全量跑都有 3~4 项环境性失败** —— Qdrant 报
  ...
  **最可疑是本机 Qdrant v1.19.0 构建**（详见 `tech.md` **§12.3**）。
...
- **本文件版本**：v0.26.1（2026-09-26 ✅ **R-38 真实验收通过（`--probe-vault` 16 项全绿）**，
  并修掉验收脚本两处"未先静止 / 未等稳定"导致的**假红**；R-42 阶段 2 把「笔记外」拆成
  "完全没提"与"提了名没解释"并扩题复测 ⇒ 建议阈值 **0.60**，而"提了名没解释"与笔记内分数带
  **深度重叠**（最高 YaRN **0.902**）⇒ **门槛在原理上拒不掉它**，只能靠答案模板（**R-47 待批准**）。
  **当前待项目工程师：① 定 `RECALL_EVIDENCE_MIN_SCORE` 是否取 0.60；② 批准 R-47（答案模板的
  "提及未解释"规则）；③ 配一份 `RECALL_API_KEYS` 把 R-40 的强制路径真跑一遍（现在只验到 S1）；
  ④ R-39 路线 + R-41 平台 + 实现细节背书**）
