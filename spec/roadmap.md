...
- ⚠️ **已定位但未解决（2026-09-25）**：**每次全量跑都有 3~4 项环境性失败** —— Qdrant 报
  ...
  **最可疑是本机 Qdrant v1.19.0 构建**（详见 `tech.md` **§12.3**）。
  📌 建议：① **换 Qdrant 稳定版本**（这是最有希望的一条）；② 降级时重启 Qdrant；
...
- **本文件版本**：v0.24.0（2026-09-25 **Qdrant 间歇性 IO 失败已定位（环境级、未解决）** + 存储泄漏清理工具。**当前待项目工程师：换 Qdrant 稳定版本 / 重启验收 + R-42 门槛 + R-39 路线 + R-41 平台 + 实现细节背书**）
...
| 2026-09-25 | — | 🔴 **Qdrant 间歇性 IO 失败：已定位、未解决（环境级）** | 复跑全量 **4 次，每次都失败**（4 / 1 / 1+1error / 4 项），签名始终一致：`500` + `Not recovered from previous error: IO Error: 拒绝访问。 (os error 5)`，失败点集中在**给新建 collection 建 payload 索引**（`index:groups`、`index:updated_at_ts`）。**逐项排除**：磁盘满（清理后余 12.6~14GB）、杀毒（Defender 实时防护**已关**）、文件系统（**NTFS**）、进程状态（**重启 Qdrant 仍复现**）、本项目改动（现象**早于**本轮改动）。**最可疑**：本机 Qdrant 为 **v1.19.0（commit `74f3e85b…`）**，比已知稳定线更新，疑似该构建在 Windows 建索引/删目录的缺陷 | ⚠️ **本轮 gates 不是全绿**（283~286 passed / 每次 3~4 项环境性失败），如实登记。**建议**：① 换 Qdrant 稳定版本；② 降级时重启它；③ 定期跑清理工具。见 `tech.md` **§12.3** |
