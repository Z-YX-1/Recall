...
- **当前步骤**：**R-42 阶段 0 测量完成（2026-09-25）** ⇒ 空档 `[0.354, 0.799]` 可分，建议门槛 **t=0.58**（保留/拒绝均 100%，R@K 与基线逐位一致）；R-40、R-38 待验收
...
- **待拍板项**：**R-42 阶段 1**（是否实施 top1 门槛、阈值取值；属可观察的行为变更）
...
- **待请示事项**（以下为**非阻塞**的后续选择）：
  - **需你拍板**：
    ...
    5. **R-42 阶段 0 结论待拍板**：测量显示空档 `[0.354, 0.799]` **可分**，建议**门槛 t=0.58**
       （笔记内保留 100% / 邻近拒绝 100% / 远域拒绝 100%，且 Recall@K 与基线逐位一致）。
       请定：**是否实施该门槛**、**阈值取值**（0.58 居中；更保守可取 0.45~0.50）。
       ⚠️ 这是**可观察的行为变更**：笔记外问题将返回**空证据**（`kb_search` 不再回一堆低相关片段），
       需回写 tech.md §4/§8 契约。详见 `eval/BASELINE.md` §7。
...
- **本文件版本**：v0.15.0（2026-09-25 **R-42 阶段 0 测量完成**：精排分数空档 `[0.354, 0.799]` 可分、
建议门槛 0.58，结论写入 `eval/BASELINE.md` §7。**当前待项目工程师：R-40/R-38 验收 + R-42 阶段 1 拍板 + R-46 处置 + 实现细节背书**）
