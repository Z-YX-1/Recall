...
- **本文件版本**：v0.26.0（2026-09-26 **R-42 阶段 2：把「笔记外」拆成"完全没提"与"提了名没解释"两类并扩题复测**
  —— 门槛负责的两类空档 `[0.392, 0.799]` ⇒ 建议值 **0.60**；而"提了名没解释"与笔记内分数带深度重叠
  （最高 YaRN **0.902**，比 28/30 道笔记内题还高）⇒ **门槛在原理上拒不掉它**，R-32d 只能靠答案模板，
  已登记 **R-47 待批准**。**当前待项目工程师：① 定 `RECALL_EVIDENCE_MIN_SCORE` 是否取 0.60；
  ② 批准 R-47（答案模板的"提及未解释"规则）；③ 重启 `recall.api` 并跑 `tools\verify_phase6.py`
  （R-38 那一项 FAIL 只因 watcher 未启动）；④ R-39 路线 + R-41 平台 + 实现细节背书**）
