...
- **本文件版本**：v0.25.1（2026-09-25 ✅ **Qdrant 升级 1.19.0 → 1.19.1，全量恢复全绿（287 passed）**；根因即该补丁修的"建 payload 索引前 flush CoW 段"。**当前待项目工程师：① 用 `tools\start-qdrant.ps1` 启动 Qdrant（本轮结束时**确认其未在运行**）→ ② 重启 `recall.api` 后跑验收脚本 + R-42 门槛 + R-39 路线 + R-41 平台 + 实现细节背书**）
